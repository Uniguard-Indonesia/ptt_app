...
public class ServerConnectTask extends AsyncTask<Server, Void, Intent> {
    ...
    protected Intent doInBackground(Server... params) {
        ...
        int audioSource = mSettings.isHandsetMode() ? MediaRecorder.AudioSource.DEFAULT : MediaRecorder.AudioSource.MIC;
        int audioStream = mSettings.isHandsetMode() ? AudioManager.STREAM_VOICE_CALL : AudioManager.STREAM_MUSIC;
        ...
        connectIntent.putExtra(HumlaService.EXTRAS_CLIENT_NAME,
                mContext.getString(R.string.app_name) + " " + applicationVersion);
        ...
        connectIntent.putStringArrayListExtra(HumlaService.EXTRAS_ACCESS_TOKENS,
                (ArrayList<String>) mDatabase.getAccessTokens(server.getId()));
