...
public class ChannelEditFragment extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if (mServiceProvider.getService() != null && mServiceProvider.getService().isConnected()) {
            // TODO: we probably should just stop this dialog in its tracks if we're
            // disconnected.
        ...
        return new AlertDialog.Builder(getActivity())
                ...
                .setPositiveButton(isAdding() ? R.string.add : R.string.save, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if (isAdding() && mServiceProvider.getService() != null
                                && mServiceProvider.getService().isConnected()) {
                            mServiceProvider.getService().HumlaSession().createChannel(getParent(),
                                    mNameField.getText().toString(),
                                    mDescriptionField.getText().toString(),
                                    Integer.parseInt(mPositionField.getText().toString()), // We can guarantee this to
                                                                                           // be an int. InputType is
                                                                                           // numberSigned.
