...
package com.uniguard.ptt_app.db;
...
public class MumlaSQLiteDatabase extends SQLiteOpenHelper implements MumlaDatabase {
    ...
    public List<Server> getServers() {
        ...
        c.moveToFirst();
        while (!c.isAfterLast()) {
            @SuppressLint("Range") Server server = new Server(c.getInt(c.getColumnIndex(SERVER_ID)),
    ...
    public void removePinnedChannel(long serverId, int channelId) {
        getWritableDatabase().delete(TABLE_FAVOURITES, "server = ? AND channel = ?",
                new String[] { Long.toString(serverId), Integer.toString(channelId) });
    }

    @Override
    public List<String> getAccessTokens(long serverId) {
        Cursor cursor = getReadableDatabase().query(TABLE_TOKENS, new String[] { TOKENS_VALUE }, TOKENS_SERVER + "=?",
    ...
    public void removeAccessToken(long serverId, String token) {
        getWritableDatabase().delete(TABLE_TOKENS, TOKENS_SERVER + "=? AND " + TOKENS_VALUE + "=?",
                new String[] { String.valueOf(serverId), token });
    }
    ...
    public boolean isCommentSeen(String hash, byte[] commentHash) {
        Cursor cursor = getReadableDatabase().query(TABLE_COMMENTS,
                new String[] { COMMENTS_WHO, COMMENTS_COMMENT, COMMENTS_SEEN },
